...
Go to `github` and create a new repo using the `ginatswarm-aws-pod-identity-webhook` aws-pod-identity-webhook. I've created
...
cd loki-app
git rm -r helm/APP-NAME-app && git commit -am "remove aws-pod-identity-webhook chart" && git push  # optionally remove the chart aws-pod-identity-webhook
